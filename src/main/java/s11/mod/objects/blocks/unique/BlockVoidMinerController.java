...
public class BlockVoidMinerController extends BlockBase {
	public static final PropertyBool ACTIVE = PropertyBool.create("active");
	public static final PropertyDirection FACING = BlockHorizontal.FACING;
	
	public BlockVoidMinerController(String name, Material material, float hardness, float resistance, String harvestTool, int harvestLevel) {
	...
	public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		if (PlayerPressing.isCrtlDown()) {
			tooltip.add(TextHelper.getLang("tile.void_miner_controller.tooltip2"));
		} else {
			tooltip.add(TextFormatting.GOLD + TextHelper.getLang("tile.void_miner_controller.tooltip"));
	...
	public TileEntity createTileEntity(World world, IBlockState state) {
		return new TileVoidMinerController();
	}
	
	@Override
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (!worldIn.isRemote) {
			playerIn.openGui(Reference.MODID, Reference.GuiIds.VOID_MINER.getIndex(), worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
		return true;
	}
}
