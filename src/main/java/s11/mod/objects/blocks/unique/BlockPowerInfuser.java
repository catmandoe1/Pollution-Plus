...
public class BlockPowerInfuser extends BlockBase {
	...
	public BlockPowerInfuser(String name, Material material, float hardness, float resistance, String harvestTool, int harvestLevel) {
	...
	public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
		if (PlayerPressing.isCrtlDown()) {
			tooltip.add(I18n.format("tile.tile_power_infuser.tooltip"));;
		} else {
	...
	//drops items in the the inventory
		...
		public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
			TilePowerInfuser te = (TilePowerInfuser)worldIn.getTileEntity(pos);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
		if (!worldIn.isRemote) {
			playerIn.openGui(Main.instance, Reference.GuiIds.POWER_INFUSER.getIndex(), worldIn, pos.getX(), pos.getY(), pos.getZ());
		}
