...
public class GuiAlloyFurnace extends GuiContainer{
	private static final ResourceLocation TEXTURES = new ResourceLocation(Reference.MODID + ":textures/gui/container/alloy_furnace.png"); // defines the gui background png
	private final InventoryPlayer player;
	private final TileAlloyFurnace tileentity;
	
	public GuiAlloyFurnace(InventoryPlayer player, TileAlloyFurnace tileentity) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		List<String> text = Lists.newArrayList(TextFormatting.LIGHT_PURPLE + "Energy Stored:" + TextFormatting.WHITE + " " + Integer.toString(tileentity.getEnergyStored()) + " / " + Integer.toString(tileentity.getMaxEnergyStored()) + " RF", 
				TextFormatting.LIGHT_PURPLE + "Process Power:" + TextFormatting.WHITE + " " + Integer.toString(PollutionPlusConfig.Machines.alloyFurnace.operationCost) + " RF/t");
	...
	protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
		...
		int alloyingScale = getAlloyingProgressScaled(37); // 1 bigger than what the image is fixes stuff
		this.drawTexturedModalRect(this.guiLeft + 74, this.guiTop + 34, 176, 2, alloyingScale, 17);
